...
@property (nonatomic) QRCodeMetadataDelegate *qrCodeMetaDelegate;
...
{
    // Add metadata output
    self.qrCodeMetaDelegate = [QRCodeMetadataDelegate new];
    self.qrCodeMeta = [AVCaptureMetadataOutput new];
    self.qrCodeMetaDelegate.delegate = delegate; // Need the QR handler to call us back
    [self.qrCodeMeta setMetadataObjectsDelegate:self.qrCodeMetaDelegate queue:dispatch_get_main_queue()];
    [self.captureSession addOutput:self.qrCodeMeta];
    self.qrCodeMeta.metadataObjectTypes = @[@"org.iso.QRCode"];
}

-(void) removeQRDetector
{
    [self.captureSession removeOutput:self.qrCodeMeta];
}
...
- (CGRect)cropRectForPreviewImage:(CIImage *)image
{
    ...
    return CGRectInset(image.extent, (image.extent.size.width-cropWidth)/2, ((image.extent.size.height-cropHeight)/2 ) + 50);
}

- (CIImage *)drawHighlightOverlayForPoints:(CIImage *)image topLeft:(CGPoint)topLeft topRight:(CGPoint)topRight bottomLeft:(CGPoint)bottomLeft bottomRight:(CGPoint)bottomRight
{
    CIImage *overlay = [CIImage imageWithColor:[CIColor colorWithRed:0 green:1 blue:1 alpha:0.6]];
...
- (void)startRectangleDetection
{
    ...
    _borderDetectTimeKeeper = [NSTimer scheduledTimerWithTimeInterval:0.1 target:self selector:@selector(enableBorderDetectFrame) userInfo:nil repeats:YES];
    ...
    [self.stillImageOutput captureStillImageAsynchronouslyFromConnection:videoConnection completionHandler: ^(CMSampleBufferRef imageSampleBuffer, NSError *error)
     {
         ...
         @autoreleasepool
         {
             ...
             if (weakSelf.enableBorderDetection && rectangleDetectionConfidenceHighEnough(_imageDedectionConfidence))
             {
...
- (CIDetector *)highAccuracyRectangleDetector
{
    ...
    dispatch_once(&onceToken, ^
    {
        detector = [CIDetector detectorOfType:CIDetectorTypeRectangle context:nil options:@{CIDetectorAccuracy : CIDetectorAccuracyHigh, CIDetectorAspectRatio : @1.73}];
    });
    return detector;
}
